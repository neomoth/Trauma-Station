...
[Serializable]
[DataDefinition]
public sealed partial class WeightedSpawnEntityBehavior : IDurabilityThresholdBehavior
{
    ...
    public void Execute(EntityUid uid, SharedDurabilitySystem system, EntityUid? cause = null)
    {
        ...
        Vector2 GetRandomVector() => new(system.Random.NextFloat(-SpawnOffset, SpawnOffset),
            system.Random.NextFloat(-SpawnOffset, SpawnOffset));

        // Randomly pick the entity to spawn and randomly pick how many to spawn
        var entity = system.Proto.Index(WeightedEntityTable).Pick(system.Random);
        ...
        if (SpawnAfter != 0)
        {
            ...
            if (!system.Proto.Resolve(TempEntityProtoId, out var tempSpawnerProto))
                return;
